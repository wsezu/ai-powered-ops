...
- **Security Reader** and **Reader**, both granted to the Function App's identity individually across all four subscriptions via `infra/modules/multi_subscription_role_assignments.bicep` (a single, generalized module taking the role ID as a parameter — not two separate, near-duplicate modules) — Security Reader for `get_security_recommendations`'s Resource Graph query, Reader for `get_advisor_recommendations`'s, since Advisor recommendations specifically require access to the resource each one is about, not just a subscription-wide security-posture role. Neither is scoped at a management group, despite that initially looking like the simpler option for both — a subscription-scoped deployment can't reach upward to grant a role at a parent management group; only downward, to subscriptions/resource groups at or below its own scope. Four individual subscription-scoped assignments per role is the correct pattern here, not a workaround for one.

## Known gaps — tracked here deliberately, not left to memory

- **The "Operational hygiene observations" format in `instructions.md` has never actually been exercised.** Zero OperationalExcellence-category Advisor recommendations exist in this environment as of writing, so that section of the instructions has been designed and reviewed, but not confirmed working against a real response. Plan: revisit in roughly a week, once more resources have been deployed or some existing resource properties have been deliberately changed specifically to trigger a real OperationalExcellence recommendation (e.g. removing a tag, disabling diagnostic settings on something Advisor tracks) — worth doing deliberately rather than waiting for one to appear on its own.
