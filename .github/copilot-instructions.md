...
2. **Infrastructure as code (`infra/`)**: subscription-scoped Bicep templates that deploy Azure resources. Entry point is `infra/main.bicep` with `infra/main.bicepparam` for configuration. It deploys resource groups (AVM), then calls `modules/supporting_resources.bicep` to deploy Log Analytics Workspace, Application Insights, User-Assigned Managed Identity, and a hardened Storage Account — all via pinned AVM modules. Shared types are in `helpers/types.bicep`, shared constants (regions, role definition GUIDs) in `helpers/variables.bicep`. See `docs/infra.md`.
