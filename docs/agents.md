...
- Confidence is explicitly tied to `data_points` and `persistence_days` — a signal flagged once is treated as a candidate to watch, not to act on.

### Tool schema requirements

Both tools in `tools.json` are declared `strict: true` in `setup_agent.py`, which means their JSON schemas must satisfy three requirements together (missing any one produces a real, if initially confusing, `invalid_function_parameters` error from the Foundry API):
...
3. Executes any requested tool calls directly against the same data functions the `GetLatestCostAnomalies`/`GetCostAnomalyHistory` HTTP endpoints use — no HTTP round-trip, since it's the same codebase
...
The agent depends on the Foundry project and model deployments defined in `infra/main.bicepparam`, and on the RBAC grants described in `docs/infra.md` (Foundry User, granted to both the Function App's identity and the CI/CD identity).
