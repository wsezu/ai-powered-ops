...
- **Azure Advisor's Resource Graph data doesn't match its own documentation samples or Portal labels — verified against real exported data, not assumed from docs.** The table name is `advisorresources`, lowercase; `AdvisorResources` (the casing most Microsoft sample queries actually use) throws a design-time "table doesn't exist" error against the real API. Separately, the Portal's "Active" recommendation status never appears in the raw data at all — the only two values observed by inspecting every row of a real export were `New` (outstanding) and `Completed`. And the Portal's "Reliability" category is stored as `HighAvailability` — confirmed against Advisor's own REST API configuration schema, which lists the category enum directly; Cost, Performance, Security, and OperationalExcellence are all unchanged from their Portal labels, only Reliability differs. Query text that looks identical to an official sample can still be silently wrong against the live API — verify field values directly against real exported data before trusting a filter, the same discipline that already caught the "category" field question for Defender assessments.
- **Never leave `openai` as an unpinned transitive dependency, even when nothing imports it directly.** `agents/cost-efficiency-advisor/requirements.txt` only ever depended on it indirectly via `azure-ai-projects`, with no explicit pin — a later `openai` release changed its own HTTP client dependency (to a package this repo had never heard of before it broke), and `deploy-agent.yml` failed with an import error that pointed nowhere near the real cause. Pin `openai` explicitly, matching whatever version `src/python/requirements.txt` uses, and don't "fix" a symptom like a missing HTTP client library by pinning the symptom's own dependency directly — that papers over the real problem and leaves it free to resurface differently next time.
- **A script that reads local files has zero awareness of what's actually merged on `origin/main`.** `setup_agent.py` reads `tools.json`/`instructions.md` relative to its own location on disk — a stale, un-pulled local checkout deploys old definitions while reporting success, with nothing anywhere pointing at the real cause. This is why it now also runs in CI (`deploy-agent.yml`), checking out `main` fresh every time. Running it locally to iterate on a change before merging is still fine — just don't treat a local run's success as confirmation of what's on `main`.
...
- **Connecting a Foundry project's Application Insights isn't enough on its own for the portal to display traces.** The connection itself (an `accounts/connections` resource) handles the agent writing traces out. Reading them back for display needs a *separate* grant: `Reader` on the App Insights resource, given to the Foundry *project's* own managed identity — not the account's, not the Function App's, a third, distinct identity not exposed by the AVM module's own outputs. Missing this shows as "Setup incomplete" in the portal even though the connection itself shows "Connected."
- **A subscription-scoped Bicep deployment cannot reach a parent management group for a role assignment — only downward, never up.** Every documented example of cross-scope deployment goes from wherever the outermost deployment command is anchored down to subscriptions/resource groups at or below it; there's no example anywhere of reaching upward. Discovered this before writing code, not after a failed deployment — if `main.bicep` (subscription-scoped) ever seems like it should assign a role at a management-group scope in one shot, it can't; do it as separate subscription-scoped assignments instead, one per subscription, same as `multi_subscription_role_assignments.bicep` already does.
- **Server-side compaction (`context_management`/`compact_threshold`) is compatible with the `conversation=`/`agent_reference` pattern, but isn't a confirmed win for it.** Triggering it is itself expensive (one observed event cost ~5x a normal turn), and every official example of actually benefiting from it assumes the caller manually drops pre-compaction items when building the next request — something the `conversation=` pattern doesn't do, since the server just replays whatever it has stored. Don't reach for this as a fix for growing-conversation cost without re-confirming the follow-up-turn cost actually drops; as of this writing, that wasn't cleanly established, and the "New conversation" button in the frontend is the actual mitigation in place.
